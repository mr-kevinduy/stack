...
@push('scripts')
    ...
    <script>
        $('#fine-uploader-manual-trigger').fineUploader({
            ...
            autoUpload: false,
            request: {
                endpoint: "{{ front_route('uploads.upload-thumbnail.store', ['code' => $code]) }}",
                customHeaders: {
            ...
            deleteFile: {
                enabled: true,
                endpoint: "{{ front_route('uploads.upload-thumbnail.destroy', ['code' => $code]) }}"
            },
            chunking: {
                enabled: true,
            ...
            callbacks: {
                onComplete: function(id, name, response) {
                    if (response.success) {
                        window.location.href = "{{ front_route('uploads.confirm', ['code' => $code]) }}";
                    }
                }
            }
        });
...
<x-layout-front>
    <x-section>
        <x-card class="w-full mx-auto">
            <x-form action="{{ front_route('uploads.upload-thumbnail.store', ['code' => $code]) }}">
                <div x-data="{ selectedTab: '{{ $uploadTypeSelected['name'] }}' }">
                    ...
                    <div class="flex items-center justify-between mt-8">
                        <a class="inline-block align-baseline font-bold text-sm text-blue-500 hover:text-blue-800" href="{{ front_route('uploads.upload-video.create', ['code' => $code]) }}">
                            Back
                        </a>
